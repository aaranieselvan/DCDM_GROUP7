...
sed 's/"[0-9]*" "//g' Disease_information_no_header.txt > Disease_information_clean.txt #remove numbers in quotations

sed 's/"//g' Disease_information_clean.txt > Disease_information_edit.txt #remove unnecessary quotations 
```
...
lines <- readLines("C:/Users/srila/Desktop/DCDM_GROUP7/Disease_information_edit.txt")
...
##Convert to csv
```{r}
write.csv(clean_disease_info, "clean_disease_info.csv", row.names = FALSE)
```

#Cleaning Parameter Description File
```{bash}
sed 's/"[0-9]*" "//g' IMPC_parameter_description_no_header.txt > IMPC_parameter_description_clean.txt

sed -i '1s/.*/impcParameterOrigId, name, description, parameterId/' IMPC_parameter_description_clean.txt  #rename column header

sed 's/"//g' IMPC_parameter_description_clean.txt > IMPC_parameter_description_edit.txt #remove unnecessary quotations 
```
...
# Read the file line by line
lines <- readLines("C:/Users/srila/Desktop/DCDM_GROUP7/originals/IMPC_parameter_description_edit.txt")

# Apply the function to each line and store results in a data frame
clean_PD <- t(sapply(lines, process_line))
...
write.csv(clean_parameter_description, "clean_parameter.csv", row.names = FALSE)
```

#Cleaning Procedure file
```{bash}
sed 's/"[0-9]*" "//g' IMPC_procedure.txt > IMPC_procedure_clean.txt

sed -i '1s/.*/impcParameterOrigId, name, description, parameterId/' IMPC_procedure_clean.txt  #rename column header

sed 's/"//g' IMPC_procedure_clean.txt > IMPC_procedure_edit.txt #remove unnecessary quotations 
```
...
# Read the file line by line
lines <- readLines("C:/Users/srila/Desktop/DCDM_GROUP7/originals/IMPC_procedure_edit.txt")

# Apply the function to each line and store results in a data frame
clean_Pro <- t(sapply(lines, process_line))
...
write.csv(clean_Pro, "clean_Pro.csv", row.names = FALSE)
```
